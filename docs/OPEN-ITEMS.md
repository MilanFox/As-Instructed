# Open Items — the work journal

**This file is a dated log, not a backlog. The live list is [`TODO.md`](../TODO.md) at the repo
root.**

What follows below is append-only history: one dated section per merge or per decision, newest at
the bottom. It is the record of why things are the way they are, and it is worth reading backwards
when something looks arbitrary. It is not a list of what is left to do, and it has not been one for
a long time.

The backlog that used to live here — design pivots, level-side fixes, engine notes, UI items,
housekeeping — was stale enough to mislead. It claimed 1402 tests, described two playtest agents as
"in flight" that finished long ago, and listed eslint as reporting two false positives when it now
reports one. Anything from it that is still true has been carried into `TODO.md`; the rest was
wrong and is gone rather than preserved.
