...
One current list. Everything here is known and not done.

`npx eslint src` reports exactly one error, a known false positive at
`src/levels/world-5/__solutions__/w5-01.ts:32` (`react-hooks/rules-of-hooks` firing on a function
named `run` that calls `use`). Leave it.

---

## Bonuses assumed to work, never proved

**Ten of the game's thirty-four bonuses have no test that earns them.**
...
- **Four sense budgets escape testing by a mechanism, not by oversight** — `w5-02/eight-probes`,
  ...
  fail loudly rather than silently. The hole is `src/levels/__tests__/levels.test.ts:380`, which
  asserts only `typeof met === 'boolean'` plus "60% of levels earn something".
...
- **`w8-05`'s par (1050) was not revisited after a seed cull dropped its two costliest instances.**
  ...
  `src/levels/world-8/w8-05.ts:1043` carries the reasoning.
...
- **The 9-tick airlock Dot names in the finale brief (`w8-05`) has no plant anywhere in Worlds
  1–7.** Plant it earlier, or drop the payoff.
- **The review's `closed` field is documented as "work orders carrying a medal".** The name is what
  invited two surfaces to print the wrong noun. Renaming it to `graded` reaches `review.ts`,
  `review.test.ts` and `game/__tests__/ungraded.test.ts`.
...
- **Board legibility is fitted by height, not width**, because the feed screen is 656x438 design
  units and a square grid is bound by its height. `w4-05` lands at 14 device px per tile at
  1280x800, half of what a width-based derivation gives and below `LEGIBLE_DEVICE_TILE_PX`. Do not
  re-derive it off width.
- **`src/ui/styles/art/signal.css` and `art/deepsite.css` still carry the old workspace's orphans.**
  They are loaded globally and hold the same dead selectors `app.css` was swept of (`.console__row`,
  `.timeline__mark`, `.viewport__now`, `.rail__label`, `.filter-group`, `.btn--next`,
  `.par-row__label`), so they carry the same collision risk. Double the blast radius: these sheets
  also style desk classes *deliberately*, and unscoped, so a rule that looks orphaned may be an art
  direction doing its job.
- **Signal restyles the board but not the desk chrome.** `src/ui/styles/art/*.css` target old-UI
  class names and `.desk` redefines `--ink`/`--accent`/`--scr` locally.
...
- **`src/ui/styles/desk/paper.css` contains a ~308-line block duplicated verbatim**, both copies
  opening `/* ==== THE SHEETS ==== */`.
- **`src/__tests__/confessed-invariants.test.ts` ends in two orphaned doc-comment blocks**
  describing tests that no longer exist, superseded by `monitor-margin.test.ts`.
- **Six CSS files cite `docs/prototypes/desk/desk.css`, which does not exist.**
...
- **`w3-04`'s star costs the medal on the two fifteen-crate seeds.** Discipline adds a survey aisle
  ...
  decision with both figures on the certificate.
...
- **Clicking below the editor's last line works.** The dead zone a playtester hit is *below the
  editor* — the OUTPUT log's header, which carries the same dark background. The honest fix is a
  visible focus state on the terminal pane, which is CSS, not a click handler.
