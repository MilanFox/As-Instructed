...
print(\`\${here.x},\${here.y} -> \${north.x},\${north.y}\`);`,
  },
  {
    ...
    title: 'Ticks and par',
    body: [
      'Sensing costs no ticks. Acting costs ticks.',
      '',
  ...
  {
    id: 'readings',
    aliases: ['reading', 'sensing', 'sense', 'allowance', 'survey', 'beam'],
    title: 'Readings',
    body: [
      'A reading is one call to a sensing function: `look`, `scan`, `canMove`, `pos`, `facing`,',
      '`clock`, `probe`, `readMark`, `inventory`, `carrying`, `capacity`, `fuel`, `fuelMax`.',
      'Anything that changes the world is an action, not a reading.',
      '',
      'One call is one reading, whatever it hands back. `look(dir, 1)` and `look(dir)` are one',
      'reading each, however far the ray runs. `scan()` reports five tiles on one reading.',
      '',
      'Readings cost no ticks, so par never counts them. Some work orders ration them anyway. Where',
      'one does, the number is in its facts, and so is which calls that number counts — every',
      'sensing call, or only the one named. The objective shows the running total against the',
      'allowance.',
    ].join('\n'),
  },
  {
