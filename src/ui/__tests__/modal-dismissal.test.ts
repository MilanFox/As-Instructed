 ...
 *
 * The hooks-and-zustand driver is `src/meta/__tests__/publish-dialog.test.ts`'s, which is what it
 * was built for: real hook semantics in node with no DOM and no new dependency. Effects are never
 * flushed here — `App`'s mount effect builds a `RuntimeRunner` and a canvas renderer, and this file
 * is about what `App` *renders*, not what it mounts.
 */
