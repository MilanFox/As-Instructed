...
const GRAMMAR = [
  ...
  '',
  '- `name` is `main` or `g1`, `g2`, `g3`.',
  '- `body` is one or more entries separated by commas.',
  '- An entry is either a **move group** — a decimal count of one or more digits followed by',
  '  one of `N`, `E`, `S`, `W`, like `12S` — or a **call** — a block name, `*`, and a decimal',
  '  repeat count, like `g2*3`. A call means: run that whole block, that many times.',
  '- Blocks nest up to four deep. A block is never called before it is defined and no block',
  '  calls itself.',
  '- `S` and `W` are the two check values over the characters of `name|body`, exactly as in',
  '  the standing signal-discipline order: writing `salt` for the number the antenna carries,',
  '  `S` is `(salt + c0 + c1 + ... + cn) mod 256` and `W` is',
  '  `(salt + 1*c0 + 2*c1 + ... + (n+1)*cn) mod 256`, where `ci` is the character code.',
].join('\n');
...
export const w6_05: LevelDef = {
  ...
  brief: [
    ...
    'station that is still sending on it uses the old nested format, which nobody has needed',
    'since the format stopped being current.',
    '',
    ...
    '2. Some blocks arrive corrupt — one character altered, checks unchanged. A corrupt block',
    '   is a lie about a block that also arrived intact. Do not act on one.',
    '3. **One block arrives shifted.** It went through the old repeater, which shifts printable',
    '   characters by a whole number from 0 to 94. `decode(text, key)` undoes a shift and costs',
    '   nothing. Its check values are computed over the plain text, not the shifted one.',
    '',
    'Blocks arrive in no particular order. Start from `main`.',
    '',
    '**Extra objective.** Repair the corrupt blocks instead of discarding them. For each one,',
    'in the order it arrived, print one line: `fix ` followed by the `name|body` it was sent',
    'as, before the character was altered.',
    '',
    '**The Repository.** The nesting is new. Everything inside the nesting is not. This work',
    'order assumes two routines are already in `lib.ts`:',
    '',
    '- `findKey(packets)` — takes the traffic and returns the shift it was sent with.',
    "  `import { findKey } from 'lib';`",
    '- `unpack(route)` — turns a run of move groups like `4E12S1W` into the moves they stand',
    "  for. A block body is those same groups with commas in it. `import { unpack } from 'lib';`",
    '',
    'If either is not in there, write it in this file — nothing here depends on their having',
    'been published. Neither of them knows what a call is, and that is the part that is yours.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
