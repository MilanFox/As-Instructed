...
export const w6_02: LevelDef = {
  ...
  brief: [
    ...
    '**TO:** Contractor #4471',
    '',
    'Signal discipline on this band is mandatory. The reason is an incident in the south field',
    'in 2207, in which an unverified packet was actioned and the field harvested itself on',
    'schedule. Memo KD-2601 covers it. It is one paragraph and it is not reassuring.',
    '',
    ...
    '',
    'The salt changes between shifts. Read it for free with `probe(\'mast\').vars.salt`.',
    'A corrupt packet has exactly one payload byte altered, so its check values do not match.',
    '',
    'Transmit every packet whose check values match, unchanged, in the order it arrived, and',
    'transmit nothing else. `transmit()` costs one tick and returns whether the antenna took it.',
    '',
    '**Extra objective.** For each corrupt packet, print one line `bad <packet> <byte>`, in the',
    'order the corrupt packets arrived. `<packet>` is the position on the band counting from 0,',
    'counting the clean packets too. `<byte>` is the position of the altered byte in that',
  ...
  starter: [
    '// Every packet is "b0,b1,...,bn*S,W". The salt is on the antenna.',
