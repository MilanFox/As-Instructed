...
export const w8_01: LevelDef = {
  ...
  brief: [
    ...
    'Crops that are still green do not count and are not worth the two ticks.',
    '',
    `The field is ${String(FIELD_W)} tiles by ${String(FIELD_H)} of open regolith. Nothing on it`,
    'blocks a sensor sweep, so a ray cast along a row reports that entire row. `probe("silo")`',
    'reports where the silo is, from anywhere, for nothing. The bot starts on it.',
    '',
    '**Two budgets apply and both are hard.** Par is the tick budget. The character budget is',
    'the length of your program once comments and leading whitespace are stripped. Missing',
    'either one is a fail. They do not pull the same way: the shortest program to write is',
    'rarely the cheapest program to run, and you will have to hold both numbers at once.',
    '',
    '**The Repository.** This work order assumes a `pathTo(x, y)` in `lib.ts`: something that',
    'walks the bot to a tile it has already seen and reports whether it arrived. Import it',
    "with `import { pathTo } from 'lib';`. If it is not in there, write it in this file —",
    'nothing here depends on its having been published, and the ticks are the same either way.',
  ].join('\n'),
  seeds: [1, 2, 3, 4],
  ...
  starter: [
    ...
    '// Ticks and characters are both graded this shift.',
    '',
  ...
  hints: [
    ...
    'Both numbers are graded. Shortening the program can lengthen the route; check the tick count again after every edit.',
  ],
