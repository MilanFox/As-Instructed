...
export const w8_04: LevelDef = {
  ...
  brief: [
    ...
    'tunnel that has since come down. The rock either side of a fallen stretch is unchanged and',
    'there is always a way round, but the plan does not know about it. The plan is right about',
    'everywhere else, including where each group of moves was supposed to finish.',
    '',
    '`look(dir, range)` is free and stops at the first thing it cannot see through. `canMove` is',
    'free and truthful. A move into rock still costs a tick.',
    '',
    ...
    '',
    '**The Repository.** Nothing in this work order is new. Every piece of it is a piece you',
    'have filed, and the only thing being asked is whether they still fit together when the',
    'ground has moved. It assumes three routines are in `lib.ts`:',
    '',
    '- `findKey(packets)` — takes the traffic and returns the shift it was sent with.',
    "  `import { findKey } from 'lib';`",
    '- `unpack(route)` — turns a run of move groups into the moves they stand for.',
    "  `import { unpack } from 'lib';`",
    '- `reach(x, y, b?)` — routes to a tile, surveying first when the record does not know it',
    "  yet. That is the one built from the two cave routines. `import { reach } from 'lib';`",
    '',
    'If any of them is not in there, write it in this file — nothing here depends on their having',
    'been published, and the ticks are the same either way.',
    '',
    'The first two read the plan. The third is what you fall back on where the plan is wrong,',
    'and it is the one that costs ticks — every one of them charged to this work order, however',
    'many files away it was written.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
