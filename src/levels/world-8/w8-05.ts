...
const BRIEF = [
  ...
  'it. Roughly a fifth of the traffic fails its checksum and is to be discarded —',
  'those packets carry coordinates that are wrong. Packets that hold carry',
  '`CRATE|x|y|kind`, `DEPOT|x|y|kind` and `FORM|x|y`. None of this is required: the',
  'same facts can be found by walking, which costs more.',
  '',
  ...
  'how many uses it takes to open. It stays open once it is open. Dot is being',
  'courteous above; the number is on the machine and can be read by program.',
  '',
  '**The deadline.** The shift is finite and the objectives panel shows the number.',
  'It is sized for a fleet that is all working at once, not for one bot doing',
  'everything in order.',
  '',
  '**The Repository.** There is nothing in this work order you have not already written',
  'and filed. It assumes three routines are in `lib.ts`, and two of them are made out of',
  'the others:',
  '',
  ...
  "  know it yet. `survey` and `pathTo`, in one. `import { reach } from 'lib';`",
  '- `dispatch(deps, costs, fleet)` — groups the grid into waves and deals each wave out',
  "  across the fleet. `waves` and `deal`, in one. `import { dispatch } from 'lib';`",
  '',
  'If any of them is not in there, write it in this file — nothing in this work order',
  'depends on your having published it earlier, and the shift is finishable start to',
  'finish by somebody who never opened the Repository. It will be a longer evening.',
  '',
  'Every tick those routines spend is charged here, the whole way down the chain: a',
  '`reach` that calls a `pathTo` that walks eleven tiles costs this shift eleven ticks.',
  'The Cost tab will tell you which of them the Kessler contract is paying for.',
  '',
