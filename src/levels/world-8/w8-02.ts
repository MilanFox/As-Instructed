...
export const w8_02: LevelDef = {
  ...
  brief: [
    ...
    '---',
    '',
    'Every crate on the floor belongs to one class, and every class has exactly one bay. Sort',
    'the depot: carry each crate to the bay for its class and drop it on that bay tile. A crate',
    'counts as delivered when it is lying on the right tile at the end of the shift.',
    '',
    ...
    'happen at once.',
    '',
    '**The Repository.** This work order assumes the two cave routines are in `lib.ts`:',
    '',
    '- `survey(b?)` — looks every way from where the bot is standing and records what it saw.',
    "  `import { survey } from 'lib';`",
    '- `pathTo(x, y, b?)` — walks the bot to a tile the record already knows, and reports whether',
    "  it arrived. `import { pathTo } from 'lib';`",
    '',
    'If either is not in there, write it in this file — nothing here depends on their having been',
    'published, and the ticks are the same either way.',
    '',
    'They are two routines and this depot wants one. Asking for a tile that is not on the record',
    'yet is not an error here; it is the normal case, and the answer is to look around and ask',
    'again. A routine that calls both of yours — survey until the tile is known, then route to',
    'it — is the shape the rest of the Contract is written against, and the Repository will offer',
    'to keep it. Later briefs call it `reach`.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
