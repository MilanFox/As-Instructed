...
export const w8_03: LevelDef = {
  ...
  brief: [
    ...
    '',
    '**A station.** `probe(id)` works on any machine from anywhere and costs nothing. It returns',
    "`at` (the station's tile), `state` (`\"off\"` or `\"on\"`) and `vars`. `vars.deps` is how many",
    'feeders that station hangs off, and `vars.dep0`, `vars.dep1` and so on are the index numbers',
    'of those feeders — `dep0: 3` means `sub-3` feeds it.',
    '',
    '**Energising.** Stand on the station tile and call `use()`. That costs two ticks. The cycle',
    'is `off, on` and it wraps, so using a station a second time turns it back off.',
    '',
    ...
    '',
    '**The deadline** is set per shift, from the shape of the grid and the size of the fleet. It',
    'scales with the depth of the grid rather than with the number of stations, and it is shown',
    'as a count against your makespan in the objectives panel after a run.',
    '',
    'The plain is open. There are no walls, and the cable on the ground is walkable. Your score',
    'is the makespan — the finish time of the last bot, not the sum of the fleet. Issuing a',
    'command to one bot does not make any other bot wait; only `wait()` and `sync()` do that,',
    'and `sync()` drags every bot in the fleet up to the clock of the one that is furthest ahead.',
    '',
    '**The Repository.** You have already filed everything this needs. It assumes three routines',
    'are in `lib.ts`:',
    '',
    '- `waves(deps)` — takes, for each station, the stations it hangs off, and groups them so',
    "  that nothing in a group waits on anything else in it. `import { waves } from 'lib';`",
    ...
    '- `pathTo(x, y, b?)` — walks a bot to a tile. The plain is open, so this one is not clever',
    "  here; it is just the thing you already have. `import { pathTo } from 'lib';`",
    '',
    'If any of them is not in there, write it in this file — nothing here depends on their having',
    'been published.',
    '',
    'The first two are the whole answer and neither of them knows it. One says what may be done',
    'at the same time; the other says who does it. A routine that calls both — group the grid,',
    'deal each group across the fleet, report who takes what and when — is a thing to keep, and',
    'the last work order of the Contract is written on the assumption that you did. Later briefs',
    'call it `dispatch`.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
