...
export const w4_05: LevelDef = {
  ...
  brief: [
    ...
    '> the cutting head will take it. the walls themselves are staying where they are. you may',
    '> take the ore. you may not widen the tunnel.',
    '',
    ...
    '`mine(dir)` to cut a piece loose. Ordinary rock cannot be cut, so there is no straight line',
    'to anything.',
    '',
    ...
    'but only while parked on the depot. The tank is a different size every shift, and it never',
    'holds enough to walk the whole cave.',
    '',
    'A `look` ray stops at the first thing it cannot see through, and it tells you what that thing',
    'was. A corridor with an ore face at the end of it can be identified from the other end of the',
    'corridor.',
    '',
    '**The Repository.** This work order assumes the two routines the last one offered to keep',
    'are in `lib.ts`:',
    '',
    '- `survey()` — looks every way from where the bot is standing and records what it saw, so',
    "  that something else can route over it later. `import { survey } from 'lib';`",
    '- `pathTo(x, y)` — walks the bot to a tile the record already knows, and reports whether it',
    "  arrived. `import { pathTo } from 'lib';`",
    '',
    'If either is not in there, write it in this file — nothing here depends on their having been',
    'published, and the ticks are the same either way. What this work order is actually asking is',
    'when to stop calling the first one, and that is not something either of them can answer.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
  ...
  starter: [
    ...
    '// mine(dir) cuts the ore face next to the bot. Ordinary rock will not cut.',
    '',
    'print(`tank: ${fuel()}`);',
    '',
  ].join('\n'),
