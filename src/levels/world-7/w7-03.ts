...
export const w7_03: LevelDef = {
  ...
  brief: [
    ...
    'wall — any tile in column 1 counts. Bots carry one crate at a time. All of it goes through',
    'the tunnel.',
    '',
    'Two bots that each retry a blocked move go on yielding to each other. Nothing moves, both',
    'keep paying a tick per attempt, and the run stops and names them. The site calls this a',
    '*sustained mutual courtesy*; see memo KD-2704. It is a fault in the schedule, and the',
    'schedule is yours.',
    '',
    ...
    '- Every action costs a fixed, published number of ticks and `move` reports whether it',
    '  worked. There is no `clock()` to read.',
    '- `wait(n)` burns n ticks on one bot only. `sync()` burns them on all of them.',
    '- `canMove(dir)` is free and truthful.',
    '',
    'Score is the makespan.',
  ].join('\n'),
  seeds: [1, 2, 3, 4],
