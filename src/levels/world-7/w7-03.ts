...
export const w7_03: LevelDef = {
  ...
  brief: [
    ...
    '',
    'Every crate stacked in the east yard has to end up in the silo, which is the whole west',
    'wall — a crate counts as delivered on any tile in column 1. Bots carry one crate at a',
    'time, so a pile of two crates is two round trips. All of it goes through the tunnel.',
    '',
    '**Read this part before you write anything.** Two bots that each retry a blocked move',
    'will each go on yielding to the other. Nothing moves, both of them keep paying a tick per',
    'attempt, and the run stops and names the bots involved. The site calls this a *sustained',
    'mutual courtesy*; see memo KD-2704. It is not a fault in the tunnel and it is not a fault',
    'in the engine. It is a fault in the schedule, and the schedule is yours. "Move, and try',
    'again if it failed" is the wrong answer here, and it is the answer you will write first.',
    '',
    'What you have to work with:',
    '',
    '- A bot that vacates a tile frees it for a bot arriving on that same tick, so bots',
    '  travelling the same way can run nose to tail one tick apart. Two bots travelling',
    '  opposite ways cannot pass at all.',
    '- Every action costs a fixed, published number of ticks and `move` tells you whether it',
    '  worked, so you can keep each bot\'s clock in an ordinary variable and know it exactly.',
    '  There is no `clock()` to read.',
    '- `wait(n)` burns n ticks on one bot only. `sync()` burns them on all of them.',
    '- `canMove(dir)` is free and truthful.',
    '',
    'Score is the makespan. A `sync()` in front of every crossing is a correct program and a',
    'slow one.',
  ].join('\n'),
  seeds: [1, 2, 3, 4],
  ...
  hints: [
    ...
    'You know every cost before the run starts, so you can work out the tick a bot will arrive at the tunnel mouth without ever asking the bot. Then the question is not "is it free now" but "when is it free".',
  ...
  docs: ['wait', 'sync', 'canMove', 'pickup', 'drop'],
};
