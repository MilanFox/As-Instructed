...
export const w7_05: LevelDef = {
  ...
  brief: [
    ...
    '',
    '- The sites are not listed. `probe()` with **no argument** reports the machine under or ahead',
    '  of the bot, and that is the only way to find one. `probe(id)` needs an id you do not have.',
    '- A bot brings a site up by standing on it and calling `use()` once. **The state wraps**:',
    '  twice puts it back to `cold`.',
    ...
    '',
    '`look(dir, range)` is free and stops at the first thing it cannot see through. A bot only',
    'knows what it has seen.',
    '',
    '**No bot may bring up a site it was not sent to.** Before a bot uses a site it must already',
    "have read a message another bot sent it. `send(to, body)` stamps a message with the sender's",
    "clock; `recv()` pops the oldest one this bot's clock has reached. A receiver running behind",
    'in virtual time sees an empty inbox.',
    '',
    'Your score is the makespan.',
    '',
    '**Extra objective.** Keep the workers off the bench: under a tenth of the shift spent',
    'waiting, across all of them.',
    '',
    '**The Repository.** This work order assumes `lib.ts` holds:',
    '',
    '- `survey(b?)` — records what a bot can see from where it stands.',
