...
export const w7_05: LevelDef = {
  ...
  brief: [
    ...
    '',
    'The relay sites in the north workings are not on any plan. They were put in by somebody',
    'who did not file, and they are still running. Field Engineering have declined to assist',
    'and have not given a reason. Dot does give reasons.',
    '',
    'Bring every relay site up. A site is up when it is in state `on`.',
    '',
    '- The sites are not listed. `probe()` with **no argument** reports the machine next to the',
    '  bot, and that is the only way to find one. `probe(id)` needs an id, and you have none.',
    '- A bot brings a site up by standing on it and calling `use()` once. **The state wraps**:',
    '  using it twice puts it back to `cold`.',
    '- `probe(\'muster\')` publishes `vars.sites`, `vars.scouts` and `vars.workers`.',
    '- The first `scouts` bot ids are your scouts. The rest are workers. They are identical',
    '  machines; the difference is what you decide to do with them.',
    '',
    'A bot only sees what it can see from where it is standing. `look(dir, range)` is free and',
    'stops at the first thing it cannot see through, so a bot that has been somewhere knows',
    'things a bot that has not been there cannot work out.',
    '',
    '**No bot may bring up a site it was not sent to.** Before a bot uses a site, it must',
    'already have read a message another bot sent it. `send(to, body)` stamps a message with the',
    "sender's clock and `recv()` pops the oldest one this bot's clock has reached. A",
    'receiver running behind in virtual time sees an empty inbox, so the working order is',
    '`send`, then `sync()`, then `recv()`.',
    '',
    'Your score is the makespan. Sending the whole fleet out to look is correct and slow.',
    '',
    '**Extra objective.** Keep the workers off the bench: less than a tenth of the shift spent',
    'waiting, across all of them.',
    '',
    '**The Repository.** Scouting an unmapped workings and sharing work out across a fleet are',
    'both things you have filed already. This work order assumes three routines are in `lib.ts`:',
    '',
    '- `survey(b?)` — looks every way from where a bot is standing and records what it saw.',
    "  `import { survey } from 'lib';`",
    '- `pathTo(x, y, b?)` — walks that bot to a tile the record already knows.',
    "  `import { pathTo } from 'lib';`",
    '- `deal(costs, fleet)` — hands a batch of jobs out, heaviest first, each to whichever worker',
    "  has the least on it so far. `import { deal } from 'lib';`",
    '',
    'If any of them is not in there, write it in this file — nothing here depends on their having',
    'been published. What none of them can do is the part this work order is about: a worker may',
    'not touch a site it was not told about, so the scout has to say so, in a message, in time.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
