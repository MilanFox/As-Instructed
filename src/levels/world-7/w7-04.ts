...
export const w7_04: LevelDef = {
  ...
  brief: [
    ...
    '**RE:** Yard 7 dispatch',
    '',
    'The board has thirty work items on it. Some of them are a minute. Some of them are the',
    'rest of the shift. The board does not distinguish between these, and neither, historically,',
    'have we.',
    '',
    'Clear the board. Every job must end in state `done` when your program stops.',
    '',
    '- Jobs are `job-0` upward. `probe(id)` is free and returns `null` past the last one. Each',
    '  one reports `vars.cost` and its position.',
    '- A bot clears a job by standing on it and calling `use()` exactly `cost` times. Each',
    '  `use()` costs one tick. **The state wraps**: one use too many puts the job back to `open`.',
    '- Bots start at Depot 0 on the west wall. The yard between is open floor.',
    '',
    'Your score is the makespan — the highest clock in the fleet, not the sum. A bot that',
    'finishes early has not helped anybody.',
    '',
    '**Extra objective.** Come in within a third of the load bound. The bound is the longer of',
    'two numbers — the longest single job, or every job plus two ticks of walking, shared out',
    "across the fleet — and then the walk out from Depot 0. `probe('board')` publishes it as",
    '`vars.bound`.',
    '',
    '**The Repository.** Nothing here needs it. But the sharing-out is not about this yard. It',
    'takes a list of costs and a number of workers and says who takes what, and it never touches',
    'a bot, a tile or a clock — which means it is the same routine on the relay sites, on the',
    'grid restart, and on whatever Finance attach to the Kessler contract. Keep it when this',
    'work order closes. Later briefs call it `deal`.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
