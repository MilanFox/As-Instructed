...
export const w7_04: LevelDef = {
  ...
  brief: [
    ...
    '',
    '- Jobs are `job-0` upward. `probe(id)` is free, returns `null` past the last one, and',
    '  reports `vars.cost` and the position.',
    ...
    '',
    '**Extra objective.** Come in within a third of the load bound: the longer of the longest',
    'single job, or every job plus two ticks of walking shared out across the fleet, plus the',
    "walk out from Depot 0. `probe('board')` publishes it as `vars.bound`.",
    '',
    '**The Repository.** Nothing here needs it. But the sharing-out never touches a bot, a tile',
    'or a clock, which makes it the same routine on the relay sites and on the grid restart.',
    'Keep it when this work order closes. Later briefs call it `deal`.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
