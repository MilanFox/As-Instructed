...
export const w7_02: LevelDef = {
  ...
  brief: [
    ...
    'It was a different level last week. It will be a different level again on Monday. Please',
    ...
    '  parent\'s clock plus the spawn cost. A bot can spawn a bot, so a chain down the apron row',
    '  raises the whole fleet without any bot walking anywhere.',
    '- Every bot carries up to 99 crops. Hauling is not part of this work order.',
    '',
    '**The Repository.** This work order assumes a `pathTo(x, y)` in `lib.ts` — the one the caves',
    "took. `import { pathTo } from 'lib';`. If it is not in there, write it in this file; nothing",
    'in this work order depends on having done it earlier.',
    '',
    'It will drive exactly one bot. The routine you filed in the caves moves *the* bot, and this',
    'apron has as many of them as Finance felt able to approve this week. The fix is a third',
    'argument — `pathTo(x, y, b)`, where `b` is a handle from `bot(id)` and the free `move` is',
    'what it falls back on when nobody passes one. The Repository will re-run every work order',
    'that already imports it, and an argument nobody passes changes nothing about any of them.',
    'That is the point of it being optional, and it is worth watching the suite prove it.',
    '',
    'Your score is the makespan — the last bot to finish. Splitting the map into equal pieces',
    'and splitting the work into equal pieces are not the same operation, and on some of these',
    'fields they give the same answer.',
  ].join('\n'),
  seeds: [1, 2, 3, 4],
