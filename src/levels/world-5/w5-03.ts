...
export const w5_03: LevelDef = {
  ...
  brief: [
    ...
    '- Each substation lists its upstream machines as `vars` keys of the form `prereq:<id>`. An',
    '  upstream may be the reactor or another substation. Some stations list none.',
    '- For **every** listed prerequisite, run `link(prereqId, stationId)`. That is the cable.',
    '- Then run `power(stationId, "on")`. A station only latches if all of its prerequisites are',
    '  already `on`. A station that lists none may be brought up at any time.',
    '- `link` and `power` cost 2 ticks each, and a futile `power` costs the same as a useful one.',
    '',
    'For the bonus: the maintenance crew walks between stations in the order you energise them.',
    'The reactor reports the allowance for that walk in `vars.travelBudget`, measured as the sum',
    'of the grid distances between consecutive stations, starting at the reactor.',
    '',
    '**The Repository.** Nothing here needs it. But whatever you write to turn that list of',
    'prerequisites into a workable order is worth keeping when this work order closes. Three',
    'later briefs hand you the same shape of graph and ask the same question, and one of them',
    'asks it of a whole fleet at once. Later briefs call it `waves`, and the useful thing for it',
    'to return is not one long order but the groups: everything that can start now, then',
    'everything that can start after those, and so on down.',
  ].join('\n'),
  seeds: [1, 2, 3, 4],
  ...
  starter: [
    ...
    '',
    '// Each station lists its upstream as vars keys named prereq:<id>.',
    '// link(prereqId, stationId) lays the cable. power(id, "on") brings it up.',
    '',
