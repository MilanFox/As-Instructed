...
export const w5_01: LevelDef = {
  ...
  brief: [
    ...
    '- **A substation only latches if the machine that feeds it is already `on`.** Switching one',
    '  early sets it to `on` and feeds it nothing, which does not count.',
    '- The substations are `sub-1` upward. `probe(id)` reads any machine anywhere in the world for',
    '  free and returns `null` for an id that does not exist.',
  ...
  starter: [
    '// The reactor is at one end of the line. The substations run away from it.',
    '// probe(id) reads any machine in the world and costs nothing at all.',
    '// use() switches the substation under the bot.',
    '',
