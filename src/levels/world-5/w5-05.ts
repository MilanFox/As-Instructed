...
export const w5_05: LevelDef = {
  ...
  brief: [
    ...
    '',
    '**The Repository.** The cabling is the work order. The bringing-up afterwards is the same',
    'question the energisation order asked, over a graph you built yourself this time: the tree',
    'you just laid *is* the dependency list. This work order therefore assumes a',
    '`waves(deps)` in `lib.ts`: something that takes, for each station, the stations it hangs',
    'off, and groups them so that nothing in a group waits on anything else in that group.',
    "Import it with `import { waves } from 'lib';`. If it is not in there, write it in this file",
    '— nothing here depends on its having been published.',
  ].join('\n'),
  seeds: [1, 2, 3, 4, 5],
